...
@OptIn(ExperimentalMaterial3Api::class) // Replace with the correct API if needed
@Composable
fun ScreenUser() {
    ...
    Column(
        ...
        Button(
            onClick = {
                coroutineScope.launch {
                    eliminarUltimoUsuario(dao) // Llamar a la función para eliminar el último usuario
                    // Opcionalmente, refrescar la lista después de la eliminación
                    val data = getUsers(dao = dao)
                    dataUser.value = data
                }
            }
...
suspend fun eliminarUltimoUsuario(dao: UserDao) {
    val lastUser = dao.getLastUser() // Obtener el último usuario
    lastUser?.let {
        dao.deleteUser(it.uid) // Eliminar el usuario por uid
    }
}